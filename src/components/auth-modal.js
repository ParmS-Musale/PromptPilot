  ...
  modal.querySelectorAll('.btn-social').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      try {
        ...
        let errorMsg = err.message || 'Social auth failed';
        if (errorMsg.toLowerCase().includes('provider') || errorMsg.toLowerCase().includes('disabled') || errorMsg.toLowerCase().includes('not enabled') || errorMsg.toLowerCase().includes('not configured')) {
          errorMsg += '. (Tip: Make sure you have enabled and configured this social provider in your Supabase Auth settings under Providers.)';
        } else {
          errorMsg += '. (Tip: Verify if this provider is enabled and configured under Authentication -> Providers in the Supabase Dashboard.)';
        }
        modalBody.innerHTML = `
